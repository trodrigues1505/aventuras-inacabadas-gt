...
export default function Planets() {
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-5 py-8 md:px-10 md:py-12">
      ...
      <WorldForm open={open} world={editing} busy={busy} onSave={save} onClose={close} />
...
function PlanetCard({
  world, done, total, pct, accent, delay, onEdit, onRemove,
}: {
  world: World
  done: number
  total: number
  ...
  return (
    ...
    >
      <div className="relative h-[140px] overflow-hidden bg-raised">
        <img
          src={`assets/planets/${world.slug ?? world.id}-banner.png`}
          alt={world.name}
        ...
        <div className="absolute right-2 top-2 flex gap-0.5 opacity-0 transition-opacity duration-150 group-hover:opacity-100 focus-within:opacity-100 max-md:opacity-100">
          <button type="button" onClick={onEdit} aria-label={`Editar ${world.name}`}
          ...
          <button type="button" onClick={onRemove} aria-label={`Excluir ${world.name}`}
        ...
        <div className="absolute bottom-0 left-0 p-3">
          <div className="flex items-center gap-2">
            <span className={`grid size-7 place-items-center rounded-[8px] text-[14px] ${accent.soft}`} aria-hidden>
              {world.icon}
            </span>
            <h2 className="text-[14px] font-semibold text-white drop-shadow">{world.name}</h2>
          </div>
        </div>
      </div>
        ...
        <div className="mt-auto">
          <div className="mb-1.5 flex items-baseline justify-between text-[12px]">
            <span className="text-faint">
              {total === 0 ? 'Sem missões' : `${done} de ${total} concluídas`}
            </span>
            {total > 0 && (
              <span className={`font-semibold tabular-nums ${accent.text}`}>{pct}%</span>
            )}
          </div>
          <div className="h-1.5 overflow-hidden rounded-full bg-raised">
            <div className={`h-full rounded-full transition-[width] duration-500 ${accent.bar}`} style={{ width: `${pct}%` }} />
          </div>
        </div>
      </div>
    </article>
...
function WorldForm({
  ...
  const initial = useMemo<WorldDraft>(
    () => world ? { name: world.name, description: world.description, icon: world.icon, accent: world.accent } : BLANK,
    [world],
  )
...
function SkeletonGrid() {
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3" aria-hidden>
      {[0, 1, 2].map((i) => (
        <div key={i} className="h-[220px] animate-pulse rounded-[16px] border border-line bg-surface" />
      ))}
    </div>
  )
}
