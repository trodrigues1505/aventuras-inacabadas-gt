...
export default function Worlds() {
  ...
  async function save(draft: WorldDraft) {
    ...
    try {
      ...
      putWorld(saved)
      toast('success', editing ? 'Mundo atualizado.' : `Mundo "${saved.name}" mapeado.`)
      close()
    } catch (e) {
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-5 py-8 md:px-10 md:py-12">
      <SectionHeader
        title="Mundos"
        note="Cada mundo agrupa um tipo de missao."
        action={
      ...
      {loading ? (
        ...
        <div className="rounded-[16px] border border-line bg-surface">
          <EmptyState
            icon={Globe2}
            title="Nenhum mundo mapeado"
            note="Mundos sao as categorias das suas missoes — trabalho, estudo, casa, o que fizer sentido para voce."
            action={
              <Button onClick={() => start()}>
                <Plus size={15} aria-hidden />
                Mapear o primeiro
      ...
      <ConfirmDialog
        ...
        message="As missoes deste mundo nao serao apagadas — elas ficam no registro sem mundo, e voce pode reatribui-las depois."
...
function WorldForm({
  ...
  const nameError = touched && !draft.name.trim() ? 'Dê um nome ao mundo.' : undefined
  ...
  return (
    <Modal
      ...
      subtitle={
        world ? undefined : 'Mundos agrupam missoes por contexto.'
      }
      footer={
        <>
        ...
        <Field label="Descricao" hint="Opcional.">
          {(id) => (
            <Textarea
              ...
              placeholder="O que voce gerencia neste mundo?"
              onChange={(e) =>
